...
### 3.2 Nota crédito *(RIPS `tipoNota` = NC · operación 20)*
- Sobre una FEV radicada. **Concepto de corrección** (tabla 13.2.4): `1` devolución parcial · `2` anulación de factura · `3` rebaja/descuento · `4` ajuste de precio · `5`/`6` descuento comercial. Referencia el **CUFE** → timbrar → transmitir MUV → ajusta cartera a la baja.
- **NC total (anulación)** → **sin RIPS**; el MUV marca la FEV como "afectada con NC al 100 %". **NC parcial** (glosa aceptada) → **con RIPS** soporte (solo registros aceptados; Σ = valor de la NC). Valor del servicio ≤ lo facturado; pagos moderadores en 0.

### 3.3 Nota débito *(RIPS `tipoNota` = ND · operación 30)*
- Ajuste al alza / cargo adicional. **Concepto** (tabla 13.2.5): `1` intereses · `2` gastos por cobrar · `3` cambio del valor · `4` otros. Referencia el CUFE → timbrar → transmitir MUV. **La ND sí incluye valores** (servicios no facturados o facturados de menos → actualiza los RIPS de la FEV); pagos moderadores en 0. Detalle: [[Notas crédito y débito (NC · ND) — flujo, campos y reglas]].
...
### 6.1 Estados de la orden / factura *(máquina de estados — dos ejes)*
- **Documento:** `borrador → generada → timbrada (CUFE) → validada (CUV) → radicada → conciliada → pagada`. **Operación** (por transición): `enviando/esperando → ok | error` (cada paso = **llamada async** a DIAN/MUV/ERP, con **loading**).
- **Excepción:** `rechazada_dian` · `rechazada_muv` (→ corregir → reintentar) · `contingencia` (DIAN no disponible → diferido) · `glosada` (→ responder) · `anulada` (vía NC total). Mapa detallado (loading, rechazos, operaciones): [[Ciclo de vida de la FEV — estados y operaciones asíncronas]].
...
- **Implementado en el prototipo (2026-07-22):** sobre una factura radicada → registrar glosa (causal + valor) → estado `glosada` → responder cada una; **aceptar genera una NC parcial** por el valor (enlaza con [[Notas crédito y débito (NC · ND) — flujo, campos y reglas]]) → **conciliar** = saldo (total − Σ aceptadas), estado `conciliada`. Causales: enum `CAUSAL_GLOSA` = **7 grupos** del **Manual de Devoluciones, Glosas y Respuestas** (MinSalud, marco FEV-RIPS · anexo de relacionamiento con RIPS, 31-may-2024, en `_Adjuntos/SISPRO FEV-RIPS/`): `FA` Facturación · `TA` Tarifas · `SO` Soportes · `AU` Autorizaciones · `CO` Cobertura · `CL` Calidad · `SA` Seguimiento a los acuerdos ✅ **verificado**. El gate vive en `reglas.dmn.json` (scope `glosa` → `RIS.validarGlosa`). *(El detalle fino son códigos `FA0101`… — 332 filas; el prototipo usa el nivel de grupo.)*
